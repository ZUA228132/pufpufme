...
export async function POST(req: NextRequest) {
  ...
  // Если есть активные выборы и уже истёк срок — определяем победителя
  if (electionRow && electionRow.status === "active") {
    const endsAt = new Date(electionRow.ends_at);
    if (endsAt.getTime() <= now.getTime()) {
      // Берём все голоса и считаем их в JS
      ...
      if (!votesErr && votesData && votesData.length > 0) {
        ...
        for (const cid of Object.keys(counters)) {
          if (counters[cid] > maxVotes) {
            maxVotes = counters[cid];
            winnerCandidateId = cid;
          }
        }

        if (winnerCandidateId) {
          // Находим юзера победителя и назначаем админом школы
  ...
  // Строим карту голосов по кандидатам (если есть выборы)
  const votesMap: Record<string, number> = {};
  let myVoteCandidateId: string | null = null;

  if (electionRow) {
    // Берём все голоса для этих выборов и считаем в JS
